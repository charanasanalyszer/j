...
function es_renderBreakList() {
  ...
  const breaks = es_state.school.breaks || [];
  if (breaks.length === 0) {
    el.innerHTML = '<div style="font-size:12px;color:var(--text3);">No breaks added</div>';
    return;
  }
  el.innerHTML = breaks.map((b,i) => `
    <div class="break-item">
      <span class="break-label">☕ After Period ${b.after} — ${b.label} (${b.duration} min)</span>
      <button class="btn btn-sm btn-danger" onclick="removeBreak(${i})">✕</button>
    </div>
  `).join('');
}
...
async function es_generateTimetable() {
  ...
  es_state.teachers.forEach(t => {
    (t.subjects||[]).forEach(sid => {
      ...
      teacherForSubject[sid].push(t.id);
    });
  });
  ...
  for (let di = 0; di < selectedClassIds.length; di++) {
    ...
    es_state.subjects.forEach(sub => {
      ...
      const teachers = teacherForSubject[sub.id] || [];
...
function es_renderTimetableView() {
  ...
  let html = `<div style="font-size:17px;font-weight:800;margin-bottom:14px;">${cls.grade}${cls.stream?' '+cls.stream:''} — Weekly Timetable</div>`;
        ...
        if (slot?.subjectId) {
          ...
          html += `<div class="tt-cell" style="padding:0;">
            <div class="tt-lesson ${slot.locked?'tt-locked':''}"
                 ...
                 draggable="true"
                 ondragstart="dragStart(event,'${classId}',${d},${p})"
                 ...
                 ondrop="dragDrop(event,'${classId}',${d},${p})">
              ...
              <div class="lesson-actions">
                <button class="lesson-action-btn" onclick="openLessonEdit('${classId}',${d},${p})" title="Edit">✏️</button>
                <button class="lesson-action-btn" onclick="toggleLock('${classId}',${d},${p})" title="${slot.locked?'Unlock':'Lock'}">${slot.locked?'🔓':'🔒'}</button>
              </div>
            </div>
          </div>`;
        } else {
          html += `<div class="tt-cell tt-empty"
                        ...
                        ondrop="dragDrop(event,'${classId}',${d},${p})"
                        onclick="openLessonEdit('${classId}',${d},${p})">
...
function es_exportPDFClass(classId) {
  ...
  doc.text(`${es_state.school.name||'School'} — ${cls.grade}${cls.stream?' '+cls.stream:''}`, 10, 15);
  doc.setFontSize(9);

  const colW  = (doc.internal.pageSize.getWidth() - 30) / (periods + 1);
  const rowH  = 15;
  let y = 22;

  doc.setFillColor(30,34,48);
  doc.setTextColor(136,146,170);
  doc.rect(10, y, colW, rowH, 'F');
  doc.text('Day', 12, y+10);
  for (let p = 0; p < periods; p++) {
    doc.rect(10 + colW*(p+1), y, colW, rowH, 'F');
    doc.text(`P${p+1}`, 10 + colW*(p+1)+2, y+10);
  }
  y += rowH;
  ...
  for (let d = 0; d < days; d++) {
    ...
    doc.rect(10, rowY, colW, rowH, 'F');
    doc.text(ES_DAY_NAMES[d], 12, rowY+10);
    for (let p = 0; p < periods; p++) {
      const slot = classTT[d]?.[p];
      ...
      if (sub) {
        ...
        doc.rect(10+colW*(p+1), rowY, colW, rowH, 'F');
        doc.setTextColor(255,255,255);
        doc.setFontSize(8);
        doc.text(sub.name.substring(0,11), 10+colW*(p+1)+1, rowY+7);
        const teacher = slot.teacherId ? es_state.teachers.find(t => t.id === slot.teacherId) : null;
        if (teacher) {
          doc.setFontSize(6);
          doc.text(teacher.name.substring(0,14), 10+colW*(p+1)+1, rowY+12);
        }
        doc.setFontSize(9);
        doc.setTextColor(220,225,235);
      } else {
        doc.setFillColor(22,25,34);
        doc.rect(10+colW*(p+1), rowY, colW, rowH, 'F');
      }
    }
  }
  doc.save(`timetable_${cls.grade}_${cls.stream||''}.pdf`);
...
function es_loadSampleData() {
  ...
  es_state.school = {
    ...
    schoolStart:'07:30',
    breaks:[
      {after:3, duration:20, label:'Short Break'},
      {after:6, duration:30, label:'Lunch Break'}
    ]
  };
  es_state.classes = [
    {id:'c1', grade:'Grade 7', stream:'East',  students:42},
    {id:'c2', grade:'Grade 7', stream:'West',  students:40},
    {id:'c3', grade:'Grade 8', stream:'East',  students:38},
    {id:'c4', grade:'Grade 8', stream:'West',  students:36},
  ...
  const mkAvail = (d, p) => {
    const a = {};
    for (let i = 0; i < d; i++) { a[i] = {}; for (let j = 1; j <= p; j++) a[i][j] = true; }
    return a;
  };
  es_state.teachers = [
    {id:'t1', name:'Mr. James Kiprono',  subjects:['s1'],       maxPerDay:6, maxPerWeek:28, availability:mkAvail(5,9)},
    {id:'t2', name:'Ms. Grace Wanjiku',  subjects:['s2'],       maxPerDay:5, maxPerWeek:22, availability:mkAvail(5,9)},
    {id:'t3', name:'Mr. David Mwangi',   subjects:['s3'],       maxPerDay:5, maxPerWeek:22, availability:mkAvail(5,9)},
    {id:'t4', name:'Ms. Faith Chebet',   subjects:['s4','s6'],  maxPerDay:6, maxPerWeek:26, availability:mkAvail(5,9)},
    {id:'t5', name:'Mr. Peter Omondi',   subjects:['s5','s7'],  maxPerDay:6, maxPerWeek:28, availability:mkAvail(5,9)},
    {id:'t6', name:'Ms. Ruth Achieng',   subjects:['s8','s12'], maxPerDay:5, maxPerWeek:20, availability:mkAvail(5,9)},
    {id:'t7', name:'Mr. Samuel Njoroge', subjects:['s10','s11'],maxPerDay:5, maxPerWeek:22, availability:mkAvail(5,9)},
    {id:'t8', name:'Ms. Ann Mutai',      subjects:['s1','s4'],  maxPerDay:6, maxPerWeek:26, availability:mkAvail(5,9)},
    {id:'t9', name:'Mr. Brian Koech',    subjects:['s9','s10'], maxPerDay:4, maxPerWeek:18, availability:mkAvail(5,9)},
    {id:'t10',name:'Ms. Esther Otieno',  subjects:['s2','s3'],  maxPerDay:5, maxPerWeek:22, availability:mkAvail(5,9)},
  ];
  es_state.rooms = [
    {id:'r1', name:'Science Lab A', type:'Science Lab',   capacity:40, subjects:['s4','s6']},
    {id:'r2', name:'Computer Lab',  type:'Computer Lab',  capacity:35, subjects:[]},
    {id:'r3', name:'Agri Plot',     type:'Agriculture Plot',capacity:45, subjects:['s8']},
    {id:'r4', name:'Art Room',      type:'Other',         capacity:35, subjects:['s9']},
  ];
  es_state.timetable = {};
  es_saveData(); es_syncSetupForm(); es_renderAll(); es_updateDashboard();
  es_toast('📦 Sample CBC data loaded! Click ⚡ Generate to create timetable.','success');
}
